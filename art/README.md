...
Character geometry uses segmented limbs weighted to the shared skeleton, with facial features, layered equipment and in-place animation. Ambient shading is painted into atlas swatches; simple directional lighting adds readable shadows. The runtime reuses one atlas GPU texture across the GLBs. First-person arms now have their own rig and idle/attack clips. Weapon sockets sit at the palms, independently oriented wrists keep bows upright, and the string/nocked arrow deform through draw and release. All poses are baked at 30 Hz with eased key poses and quaternion rotations. Third-person arms use analytic two-bone IK during authoring; the runtime only plays the exported clips. The 35% contact marker is retimed to the simulation windup, including when balance is reloaded mid-attack. Simulation timing alone decides hits.
...
Close models are 3,256–3,680 triangles; commander variants are 1,296–1,453. This leaves headroom below the 8,000-triangle target for later sculpting and animation refinement. The current style is a deliberately economical tabletop treatment, not the full detail of the concept paintings.
