...
Close characters use the detailed asset within 12 metres of the controlled unit; commander/distant characters use the lower-detail variant. Reusable actions and model geometry share each Blender source; replacing a weighted mesh preserves the skeleton animation. Actual travel drives gait phase. Seeded attack variants capture their windup, recovery and contact markers in simulation; additive hits and directional falls remain cosmetic. Animation never decides damage. See `art/README.md` for editing/export and `docs/ANIMATION_QA.md` for the current motion contract. Desktop uses one restrained shadow-casting sun; touch mode omits dynamic shadows and limits pixel ratio. Menus, results and paused views render on invalidation only. Settings persist locally; reduced motion limits weapon movement and disables cosmetic corpses.
